...
namespace
{
    ...
    void test()
    {
        try
        {
            ...
            game::ResourceLoader resource_loader{"../assets/"};
            auto camera = game::Camera{{0.0f, 0.0f, 6.0f},
                                       ...
                                       0.1,
                                       100.0f};
            game::SceneManager manager{window};
            
            manager.add_scene<game::ClearColorScene>("ClearColorScene");
            manager.add_scene<game::LightningScene>("LightningScene", resource_loader, &window, &camera);
                    ...
                    std::visit(
                        [&](auto&& arg)
                        {
                            ...
                            else if constexpr (std::is_same_v<T, game::MouseButtonEvent>)
                            {
                                ...
                                io.AddMouseButtonEvent(0, arg.get_state() == game::MouseButtonState::DOWN);
                            }
                        },
                        *event);
...
int main(int argc, char** argv)
{
    ...
    //     //     {
    //     //         entities.emplace_back(&mesh, &material,
    //     //                               game::Vector3{static_cast<float>(i) * 2.5f, dist(gen), static_cast<float>(j)
    //     //* 2.5f}, game::Vector3{1.0f},
    //     //                               tex_samp);
    //     //     }
    //     // }
